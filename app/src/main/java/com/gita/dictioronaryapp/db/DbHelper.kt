...
class DbHelper private constructor(context: Context) :
    ...
    fun getWordByQuery(str: String): ArrayList<Dictionary> {
        ...
        var cursor = getDatabase().rawQuery(
            "select * from dictionary where dictionary   .english like '%$str%'",
            null
        )
    ...
    fun getWordByQueryUZb(str: String): ArrayList<Dictionary> {
        var wordsList = ArrayList<Dictionary>()

        var cursor = getDatabase().rawQuery(
            "select * from dictionary where dictionary.uzbek like '%$str%' or dictionary.english like  '%$str%'",
            null
        )
        if (cursor.moveToFirst()) {
            do {
    ...
    fun getAllWords(offset: Int): ArrayList<Dictionary> {
        var wordsList = ArrayList<Dictionary>()
        val cursor =
            getDatabase().rawQuery("select * from dictionary limit 10 offset $offset", null)
        if (cursor.moveToFirst()) {
            do {
                val id = cursor.getInt(0)
                val english = cursor.getString(1)
    ...
    fun getAllBookmarksEng(): ArrayList<Dictionary> {
        var wordsList = ArrayList<Dictionary>()
        val cursor =
            getDatabase().rawQuery("select * from dictionary where is_favourite = 1 ", null)
        if (cursor.moveToFirst()) {
            do {
                val id = cursor.getInt(0)
                val english = cursor.getString(1)
